...
**Ref: `distributed-systems-mandate.md`**

**Idempotency:**
- All `POST`/`PATCH` endpoints **MUST** support `Idempotency-Key` header.
- Retry safe: If the client retries with the same key, return the previous successful response.

**RPC vs REST:**
- **REST**: Resource-oriented (User, Order). Best for public APIs.
- **gRPC/Connect**: Action-oriented. Best for internal microservices requiring performance and type safety.
- **GraphQL**: Graph-oriented. Best for complex frontend data fetching to avoid over-fetching.

**Long-Running Operations:**
- Return `202 Accepted` with `Location` header to a status polling endpoint.
- **Never** block the request thread for >500ms.
